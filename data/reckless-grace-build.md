...
- `algenib-crown-of-the-sky` + `let-harmony-begin` — **FIXED (2026-09-07)**. Brian confirmed the DOWNLOADED wav/mp4 (4:43 / 5:14) are the CORRECT take; the pre-selected site mp3 (5:45 / 6:48) was the WRONG longer take. Fix: transcoded correct wav→mp3 (replaced site mp3), ingested correct wav/mp4/stems→R2, re-transcribed correct audio→whisper-cache→re-aligned karaoke (100%)→replaced stale data.ts lyrics, added `durations.ts` entries (283 / 314). **Queue now 129/129 done.** Deployed + prod-verified (live mp3 4:43 / 5:14).
- **New Reckless Grace cover** (Brian, 2026-09-07): regenerated `cover-reckless-grace.jpg` as "cargo ships off Gaza" (gpt-image-1, 236KB) → RG album + 20 track OG cards regenerated.
- **Lesson**: `data/whisper-cache/<id>.json` MUST carry a top-level `duration` (from whisper verbose_json) or `align` writes `duration:0` → validate "past duration 0s". `align`/`clamp` with no args reprocess the WHOLE catalog + can degrade curated tracks (the-soul-key, hate-dies-here) — always scope to the changed ids (`align <id>…`, `clamp <id>…`), restoring others via `git checkout -- public/lyrics/`.

...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (isAudio) {
      ...
      // #v4: bump on catalog changes that delete/rename audio so stale
      // Cache API entries (30d immutable) can't ghost-serve removed files.
      const cacheKey = new Request(`${url.origin}${url.pathname}#v4`, { method: 'GET' });
    ...
    if (url.pathname.startsWith('/lyrics/') && url.pathname.endsWith('.json')) {
      const cache = (caches as unknown as { default: Cache }).default;
      // #v4: bump on catalog changes that delete/rename tracks so stale
      // Cache API entries (30d immutable) can't ghost-serve removed files.
      const cacheKey = new Request(`${url.origin}${url.pathname}#v4`, { method: 'GET' });
